...
df_cleaning <- df_cleaning %>%
  ...
  # Arrangin and Grouping for clarity
  arrange(ticker, end) %>%
  group_by(ticker) %>%
...
# Check the result
table(df_cleaned$is_overperformer)



# Creating the Future Performance prediction variable
df_predictive <- df_cleaned %>%
  arrange(ticker, year) %>%
  group_by(ticker) %>%
  mutate(
    # Selecting the "is_overperformer" from the NEXT year
    target_next_year = lead(is_overperformer, order_by = year),
    # Ebit Margin prediction
    ebit_margin_pred = lead(ebit_margin_w, order_by = year)) %>% 
  ungroup() %>%
  
  # Remove the most recent year for each company
  filter(!is.na(target_next_year))

# Check the result
# You will lose 1 row per company (the most recent year), which is expected.
print(nrow(df_cleaned) - nrow(df_predictive))
# The task was successful, if the amount of tickers matches the printed value
df_cleaned %>%
  summarise(total_companies = n_distinct(ticker))


# FINAL DATASET TO BE USED
df_final <- df_predictive %>% select(
  -ebit_margin,
  -asset_turnover,
  -rnd_per_revenue,
  -capex_margin,
